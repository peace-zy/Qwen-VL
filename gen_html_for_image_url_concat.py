...
def txt2html(gt_file, res_file, office_res_file):
    """txt2html"""
...
'''
    ...
    wlines.append('<tr><td>改造前</td><td>改造后</td><td>居住需求</td><td>设计理念</td><td>自建-生成结果</td><td>Qwen-office生成结果</td></tr>\n')
    ...
    with open(office_res_file, "r") as f:
        office_res_data = json.load(f)
    
    with open(gt_file, 'r') as f:
        gt_data = json.load(f)
    pre_fix = "http://localhost:8000/data/nfs-ten9/nfs/zhangyan461"
    pre_fix = "http://10.201.225.29:8080/home/zhangyan461/data/nfs-ten9/nfs/zhangyan461"
    out_data = []
    for idx, ele in enumerate(gt_data):
        ...
        for item in office_res_data:
            if item["image_id"] == image_id:
                office_predict = item["caption"]
                break
        if office_predict is None:
            print("predict is None")
        ...
        wl += '<td>{}</td><td>{}</td><td>{}</td><td>{}</td>'.\
                format(demand, idea, predict, office_predict)
        cnt += 1
        out_data.append([before_img_url, after_img_url, demand, idea, predict, office_predict])
        if cnt == COL_NUMS:
            wl += '</tr>\n'    
            #print(cnt, 'e')
            cnt = 0
    ...
    df = pd.DataFrame(out_data, columns=['改造前', '改造后', '居住需求', '设计理念', '自建-生成结果', 'Qwen-office生成结果'])
    #df = pd.DataFrame(out_data)
    #df.to_csv('output_duibi.csv', index=False)
    df.to_excel('结果对比.xlsx', index=False)
...
def main():
    ...
    gt_file = sys.argv[1]
    res_file = sys.argv[2]
    office_res_file = sys.argv[3]

    txt2html(gt_file, res_file, office_res_file)
    return
