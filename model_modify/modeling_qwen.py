...
class QWenLMHeadModel(QWenPreTrainedModel):
    ...
    def chat_batch(
        ...
        **kwargs,
        ...
        input_ids = torch.tensor(context_tokens_list).to(self.device)
        outputs = self.generate(
                    input_ids,
                    stop_words_ids=stop_words_ids,
                    return_dict_in_generate=False,
        ...
        response_list = []
        for idx, ele in enumerate(outputs):
            raw_text = raw_text_list[idx]
            context_tokens = context_tokens_list[idx]
            response = decode_tokens(
                ele,
                tokenizer,
                raw_text_len=len(raw_text),
