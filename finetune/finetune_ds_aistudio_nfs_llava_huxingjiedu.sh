...
DATA="/aistudio/workspace/qwen_train/dataset/qwen_format_train_二手房20549052.json"
check ${DATA}
...
${TORCHRUN_BIN} $DISTRIBUTED_ARGS finetune.py \
    ...
    --bf16 True \
    --fix_vit True \
    --output_dir output_qwen_new_llava_huxingjiedu_18w \
    --num_train_epochs 1 \
    ...
    --log_level "debug" \
    --deepspeed finetune/ds_config_zero2.json > ${LOG_PATH}"/"llava_${LOG_NAME}".log" 2>&1
