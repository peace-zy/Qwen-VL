...
DATA="/aistudio/workspace/qwen_train/dataset/sekuai_des/qwen_format_train_sekuai_des.json"
check ${DATA}
...
SUB_NAME=sekuai_des_align_zero2_a800_vit_on_lm_off_cosine_2048_ep1
${TORCHRUN_BIN} $DISTRIBUTED_ARGS finetune_fix_lm.py \
    --model_name_or_path $MODEL \
    --data_path $DATA \
    --bf16 True \
    --fix_vit False \
    --fix_lm True \
    --output_dir "output_qwen_"${SUB_NAME} \
    --num_train_epochs 1 \
    ...
    --log_level "debug" \
    --deepspeed finetune/ds_config_zero2_sekuai_des_align_a800_cosine.json > ${LOG_PATH}"/"${SUB_NAME}"_"${LOG_NAME}".log" 2>&1
